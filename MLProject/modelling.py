...
def main():
    ...
    with mlflow.start_run() as active:
        mlflow.set_tags({'stage': 'ci-retraining', 'student': 'DaudHidayatRamadhan',
        ...
        metrics = {**scores(train.target, model.predict(train[['Sentence']]), '', training=True),
        ...
        signature = mlflow.models.infer_signature(test[['Sentence']].iloc[:3], model.predict(test[['Sentence']].iloc[:3]))
    ...
    assert len(actual) == 32 and actual == expected, 'Exported model prediction mismatch'
    assert np.array_equal(mlflow.sklearn.load_model(str(model_dir)).predict(sample), expected)
    run = mlflow.get_run(run_id)
    assert run.info.status == 'FINISHED'
    for key, value in metrics.items():
        assert np.isclose(run.data.metrics[key], value), key
    (output/'inference_fixture.json').write_text(json.dumps({'dataframe_split': sample.to_dict(orient='split'), 'expected': expected}, indent=2)+'\n')
    (output/'processed_manifest.json').write_text(json.dumps(data_manifest, indent=2)+'\n')
    manifest = {'run_id': run_id, 'status': run.info.status, 'source_commit': os.environ.get('SOURCE_COMMIT', 'local-preflight'),
                'github_run_id': os.environ.get('GITHUB_RUN_ID', ''), 'metrics': metrics,
                'data_sha256': data_manifest['raw_sha256'], 'selected_C': 2.0, 'pyfunc_roundtrip': 'pass',
