...
with tempfile.TemporaryDirectory(prefix='msml-artifact-') as temp:
    ...
    exists = git('ls-remote', '--heads', 'origin', 'model-artifacts', cwd=root, capture=True).stdout.strip()
    if exists:
    ...
    (root/'latest.json').write_text(json.dumps({'path': f'models/{run_key}', 'source_commit': manifest['source_commit'], 'mlflow_run_id': manifest['run_id'], 'github_run_id': os.environ['GITHUB_RUN_ID']}, indent=2)+'\n')
    (root/'README.md').write_text('# Verified CI model artifacts\n\nEach models directory is a distinct CI run. latest.json points to the most recently persisted successful run. Verify manifest.json hashes before inference. Source code is on main.\n')
    git('add', '.', cwd=root)
